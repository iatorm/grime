...
- `PoS`, where `S` is a (greedily parsed) string of the characters `ABCDEFGH`, is an _orientation modifier_. Each of the characters stands for a rotation or reflection, which is applied to the expression `P`. The transformed expressions are combined with conjunctions. The characters `OXNTK` encode convenient classes of orientations.
